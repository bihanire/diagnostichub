...
class _JobCardPDF(FPDF):
    ...
    def _section_title(self, text: str, y: float | None = None) -> None:
        ...
        self.cell(0, 5, text.upper(), fill=True, ln=True, border=0)
        self.ln(1)
    ...
    def _stub(self, label: str, y: float) -> None:
        ...
        self.set_xy(14, y + 2)
        self.cell(40, 4, label, ln=False)

        self.set_text_color(*DARK)
        self.set_font("Helvetica", "B", 9)
        self.set_xy(14, y + 7)
        self.cell(80, 5, c.reference, ln=False)

        self.set_font("Helvetica", "", 7)
        self.set_text_color(*MID)
        self.set_xy(100, y + 2)
        self.cell(50, 4, f"{c.client_name}  ·  {c.client_phone}", ln=False)

        self.set_xy(100, y + 7)
        self.cell(50, 4, f"{c.device_model}  ·  {c.device_imei}", ln=False)

        self.set_xy(158, y + 2)
        self.set_font("Helvetica", "B", 7)
        self.set_text_color(*WATU_BLUE)
        label_type = CASE_TYPE_LABELS.get(c.case_type, c.case_type.upper())
        self.cell(38, 4, label_type, align="R", ln=False)

        self.set_xy(158, y + 7)
        self.set_font("Helvetica", "", 6.5)
        self.set_text_color(*MID)
        date_str = _fmt_dt(c.created_at).split("  ")[0]
        self.cell(38, 4, date_str, align="R", ln=False)
    ...
    def _build_copy(self, copy_label: str, y_start: float) -> float:
        ...
        self.cell(100, 10, c.reference, ln=False)

        self.set_font("Helvetica", "B", 8)
        self.set_xy(16, y_start + 8.5)
        label_type = CASE_TYPE_LABELS.get(c.case_type, c.case_type.upper())
        self.cell(100, 4, label_type, ln=False)

        self.set_font("Helvetica", "", 7)
        self.set_xy(130, y_start + 2)
        self.cell(64, 4, f"EC: {c.ec_location.name}", align="R", ln=False)
        self.set_xy(130, y_start + 6.5)
        self.cell(64, 4, _fmt_dt(c.created_at), align="R", ln=False)
        self.set_xy(130, y_start + 11)
        self.set_font("Helvetica", "B", 7)
        self.set_text_color(255, 215, 80)
        self.cell(64, 4, copy_label.upper(), align="R", ln=False)
        ...
        if c.sym_code or c.defect_description:
            ...
            if c.defect_description:
                self.set_font("Helvetica", "", 7)
                self.set_text_color(*MID)
                self.cell(0, 4, "Defect Description", ln=True)
                self.set_text_color(*DARK)
                self.set_font("Helvetica", "B", 8)
        ...
        if c.lock_type == "pin" and c.client_pin:
            self.set_font("Helvetica", "", 7)
            self.set_text_color(*MID)
            self.cell(0, 4, "Client PIN", ln=True)
            self.set_text_color(*DARK)
            self.set_font("Helvetica", "B", 8)
            self.cell(0, 4.5, c.client_pin, ln=True)
            self.ln(2)
        if c.lock_type == "pattern" and c.pattern_sequence:
            self.set_font("Helvetica", "", 7)
            self.set_text_color(*MID)
            self.cell(0, 4, "Pattern Sequence", ln=True)
            self.set_text_color(*DARK)
            self.set_font("Helvetica", "B", 8)
            self.cell(0, 4.5, c.pattern_sequence, ln=True)
            self.ln(2)
        ...
        self.cell(88, 4, "EC Agent Signature & Name", ln=False)
        self.set_xy(100, sig_y)
        self.cell(98, 4, "Client Signature", ln=True)
        self.ln(5)
        sig_y2 = self.get_y()
        self.set_draw_color(*BORDER)
